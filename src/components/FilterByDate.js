...
const FilterByDate = React.forwardRef((props, ref) => {
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        const data = {
          startDate: !!values.range[0].startDate
            ? format(values.range[0].startDate, "yyyy-MM-dd")
            : null,
          endDate: !!values.range[0].endDate
            ? format(values.range[0].endDate, "yyyy-MM-dd")
            : null,
        };
        ...
        return (
          <form onSubmit={handleSubmit} ref={ref}>
            ...
            <div className="flex gap-1">
              ...
              <button
                type="submit"
                className="px-4 py-2 m-0 text-white rounded-md btn-form-primary h-fit bg-primary"
              >
                Apply
              </button>
            </div>
